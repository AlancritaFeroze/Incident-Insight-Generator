...
def test_getCoordinates():
    ...
    latitude, longitude = util.retrieve_coord(address, reference_latitude, reference_longitude)
...
def test_DayAndHour():
    ...
    day_of_week = util.retrieve_day_week(date_string)
    hour = util.extract_hr_from_ts(date_string)
...
def test_get_town_side():
  ...
  assert util.get_side_of_town(latitude, longitude, reference_latitude, reference_longitude) == "NW"
