...
cache = {}
def retrieve_coord(address,reference_latitude,reference_longitude,max_retries=2):
    retries = 0
...
def retrieve_day_week(date_string):
...
def extract_hr_from_ts(timestamp):
    # Assuming timestamp is in the format "YYYY-MM-DD H:MM"
    date_obj = datetime.strptime(timestamp, "%m/%d/%Y %H:%M")
    hour = date_obj.hour
    return hour
def get_side_of_town(latitude, longitude, reference_latitude, reference_longitude):
...
def retrieve_weather(latitude,longitude,timestamp):
    latitude = int(latitude)
    longitude = int(longitude)
    date = get_date_from_ts(timestamp)
    key = (latitude,longitude,date)
    if key in weatherCache:
        weatherCodes = weatherCache[key]
        hour = extract_hr_from_ts(timestamp)
